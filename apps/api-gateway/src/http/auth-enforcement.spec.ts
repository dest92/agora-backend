...
describe('Auth Enforcement', () => {
  ...
  it('should only have @Public() decorator on allowed endpoints', () => {
        ...
        if (publicMatches.length > 0) {
          ...
          lines.forEach((line, index) => {
            if (line.includes('@Public()')) {
              inPublicMethod = true;
            } else if (inPublicMethod && line.match(/@(Get|Post|Put|Delete|Patch)\(/)) {
              ...
              methodPath = `/${controllerPath}${path ? '/' + path : ''}`.replace(/\/+/g, '/');
              if (methodPath === '/') methodPath = '/health'; // Special case for root health
              
              // Check if this endpoint is allowed to be public
              const isAllowed = allowedPublicEndpoints.some(allowed => 
                methodPath === allowed || methodPath.startsWith(allowed)
              );
              
              if (!isAllowed) {
                violations.push(`${file}: ${method.toUpperCase()} ${methodPath} should not be @Public()`);
              }
    ...
    if (violations.length > 0) {
      throw new Error(`Auth enforcement violations found:\n${violations.join('\n')}`);
    }
  });
  ...
  it('should protect endpoints that require authentication', () => {
    ...
    protectedEndpoints.forEach((file) => {
      try {
        ...
        if (hasPublic) {
          throw new Error(`${file} should not have @Public() decorators - all endpoints should be protected`);
        }
      } catch (error) {
